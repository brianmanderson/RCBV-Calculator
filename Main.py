...
MRN = 'some_number'
contour_path = r'Z:\Carlo rCBV Files\Tumor and White Matter\{}\Pre\Tumor_RayStation_Export'.format(MRN)
perfusion_path = r'Z:\Carlo rCBV Files\Tumor and White Matter\{}\Pre\Perfusion_Raystation_Export'.format(MRN)

out_path = r'Z:\Carlo rCBV Files\Tumor and White Matter\{}\Pre\Tumor_RayStation_Export'.format(MRN)
'''
...
reader.down_folder(contour_path)

MR_numpy_array = reader.ArrayDicom
tumor_numpy_array = reader.mask
# Save them as .nii files
sitk.WriteImage(reader.annotation_handle, os.path.join(out_path, 'tumor_mask.nii'))
sitk.WriteImage(reader.dicom_handle, os.path.join(out_path, 'T1_plus_c_Image.nii'))

'''
Now load up the Perfusion image
'''
reader_perfusion = DicomReaderWriter(get_images_mask=True)
reader_perfusion.down_folder(perfusion_path)
...
sitk.WriteImage(resampled_perfusion, os.path.join(out_path, 'resampled_perfusion.nii'))
